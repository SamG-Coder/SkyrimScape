 ...
 for(std::size_t i=0;i<48;++i){
  ...
  if(visible&&camera&&i<snapshot.size()){
   ...
   if(age>=0&&age<1.25f&&camera->WorldPtToScreenPt3(p.position,x,y,z,1e-5f)&&std::isfinite(x)&&std::isfinite(y)&&x>=0&&x<=1&&y>=0&&y<=1){
    ...
    if(!exists||rendered[i]!=key){vector_text::draw(label,p.text,3.,p.block?0x71D9FF:p.player?0xFF6868:0xFFE8A0);rendered[i]=key;}
    info.SetPosition(rect.left+x*(rect.right-rect.left)-p.text.size()*9.+(static_cast<int>(p.lane)-1)*20,rect.top+(1-y)*(rect.bottom-rect.top)-age*48-p.lane*14);
